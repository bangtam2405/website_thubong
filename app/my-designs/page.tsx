...
export default function MyDesignsPage() {
  ...
  const handleCopy = async (d: any) => {
    // Gọi API POST với dữ liệu thiết kế cũ, đổi tên
  ...
  const handleShare = async (d: any) => {
    // Gọi API PUT để set isPublic: true
    await axios.put("http://localhost:5000/api/designs", { id: d._id, isPublic: true });
    window.location.reload();
  };
        ...
        {designs.map(d => (
          <li key={d._id} className="border rounded-lg p-4 flex flex-col md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <div className="text-sm text-gray-500">Cập nhật: {d.updatedAt ? new Date(d.updatedAt).toLocaleString() : ""}</div>
            ...
            <div className="flex gap-2 mt-2 md:mt-0">
              ...
              <button className="px-3 py-1 bg-green-500 text-white rounded" onClick={() => handleShare(d)}>Chia sẻ</button>
